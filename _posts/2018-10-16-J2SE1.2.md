...
> Java集合中的TreeSet和TreeMap，C++ STL中的set、map，以及Linux虚拟内存的管理，都是通过红黑树去实现的。（MySQL索引是B+树）
...
**B树多用在外排**（内存里放不下，大部分数据存储在外存上时），因为**B树层数少**，因此可以确保每次操作，读取磁盘的次数尽可能的少。B树是为了提高磁盘或外部存储设备查找效率而产生的一种多路平衡查找树。
