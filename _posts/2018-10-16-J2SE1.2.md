...
首先先介绍下keySet和entrySet方法。
> 1. keySet顾名思义就是返回集合中的所有key，以set集合方式返回。
> 2. entrySet就是返回集合中的所有Entry节点，也是以Set集合方式返回。
> 3. 两个方法的内部都是实例化了一个AbstractSet对象，并实现了iterator/size/contains/remove/clear方法
...
HashIterator实现了Iterator迭代器，**支持3种类型的迭代（KEYS/VALUES/ENTRIES）**。接下来看下HashIterator是如何实现hasNext/next/remove方法的。
