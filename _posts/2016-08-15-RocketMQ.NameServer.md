...
RocketMQ主要由Producer、Concumer、NameServer、Broker四个部分组成。NameServer用于管理Broker，接受Broker的注册并与其进行心跳检测维护。Broker用于存储消息，多启动多个Broker同步备份消息，并将自己注册到NameServer。Producer从NameServer选择一个Broker发送消息。Consumer向NameServer订阅消息。
